...
/**
 * 
 * DAO Implementation making use of PDO API
...
{//------------------------->> Class Manager
    ...
    {//-------------------->> __construct()
        ...
        $this->setPrimaryKey( ( isset( $config[ 'primaryKey' ] ) ) ? $config[ 'primaryKey' ] : 'id' );
    ...
    {//-------------------->> _executeStatement()
        ...
        if ( !$this->_pdoStatement->execute() && 
            ...
            throw new DAO_Exception( "Error Saving Data: {$errorData[ 2 ]}" );
    ...
    {//-------------------->> fetch()
        ...
        $this->_sqlQuery = "SELECT {$fieldsList} FROM {$this->_tableName}{$whereSentence}{$groupBySentence}{$havingSentence}{$orderBySentence}{$limitSentence};";
        $this->_interceptFetchAction();
     ...
     * <p>The placeholders of the sentence will be placed in form of :fieldName</p>
